...
import { CadastroVagaService } from '../../service/cadastro-vaga.service';
@Component({
...
export class CadastroVagaComponent {
  ...
  constructor(
    ...
    this.escolaridade = [
      ...
      {
        idEscolaridade: 3,
        descEscolaridade: 'Ensino Medio Completo Incompleto',
      },
  ...
  vagaForm = this.formBuilder.group({
    ...
    infoContrato: ['', Validators.required],
    regimeContrato: ['', Validators.required],
  ...
  postVaga(events$: any) {
    ...
    this.cadastroVagaService.postVaga(events$).subscribe({
      next: (valores: any) => {
  ...
  deleteVaga(arg0: string | null | undefined) {
    if (!arg0) {
      this.mensagem.erro('Vaga não encontrada');
      return;
    }
    console.log('deleteVaga', arg0);
    this.cadastroVagaService.deleteVaga(Number(arg0)).subscribe({
      next: (response: any) => {
        console.log('Vaga excluída com sucesso', response);
        this.mensagem.sucesso('Vaga excluida com sucesso');
        this.carregaGrid();
        this.vagaForm.reset();
      },
      error: (error: any) => {
        console.error('Erro ao excluir vaga', error);
        this.mensagem.erro('Erro ao excluir vaga');
      },
    });
  }
  carregaGrid() {
    this.isLoadingResults = true;
    this.cadastroVagaService.getVaga().subscribe({
      next: (data: any) => {
        if (Array.isArray(data)) {
  ...
  recebeValores($event: any) {
    ...
    this.vagaForm.patchValue({
      ...
      infoContrato: $event.info_contrato,
      regimeContrato: $event.regime_contrato,
